// Class/ClassPage.tsx - Fully Responsive
...
export default function ClassPage() {
  ...
  const handleSave = async (data: {
    ...
    try {
      ...
      Swal.fire({
        title: "Error!",
        text: error instanceof Error ? error.message : "Failed to save class",
        icon: "error",
      });
    }
  };
  ...
  const handleDelete = async (classId: number, className: string) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Error!",
        text: error instanceof Error ? error.message : "Failed to delete class",
        icon: "error",
      });
    }
  };
    ...
    <div className="relative h-full overflow-y-auto bg-transparent selection:bg-yellow-400 selection:text-green-950">
      ...
      <div className="relative z-10 p-3 sm:p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
        {loading ? (
          <div className="flex items-center justify-center p-8 sm:p-12">
            <div className="text-white text-center">
              ...
              <p className="text-sm sm:text-base">Loading classes...</p>
            </div>
          </div>
        ) : classes.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-8 sm:p-12 text-center rounded-2xl sm:rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl">
            <p className="text-green-200/60 text-base sm:text-lg">No classes found. Add your first class to get started!</p>
          </div>
        ) : (
        ...
        >
          ...
          <span className="hidden xs:inline">Add New Class</span>
