...
export function StudentSummary({ data }: { data: DashboardData['studentSummary'] }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
      <StatsCard label="Total Students" value={data.total} icon={<Users size={20} />} />
      ...
      {data.newAdmissions !== undefined && (
        <StatsCard label="New Admissions" value={data.newAdmissions} icon={<Users size={20} />} color="text-purple-400" />
      )}
    </div>
  );
}