// components/Student/BatchModal.tsx
import { useEffect, useState } from "react";
import { X, Layers3, Building2, Calendar, Clock } from "lucide-react";
...
export default function BatchModal({
  ...
  useEffect(() => {
    ...
    const fetchModalData = async () => {
      ...
      try {
        ...
        setCampusName(campusData.campus_name);
  ...
  const populateEditData = () => {
    if (editData) {
      setBatchName(editData.batch_name || "");
  ...
  const isValid = batchName && campusName && startDate && endDate && shift;
      ...
      >
        ...
        <div className="px-4 sm:px-8 pt-6 sm:pt-8 pb-4 sm:pb-6 text-center flex-shrink-0">
          ...
          <p className="text-green-100 mt-1 sm:mt-2 text-sm sm:text-base">
            {mode === 'edit' ? 'Update batch information' : 'Add batch information and academic details'}
          </p>
        </div>
